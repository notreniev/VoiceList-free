 ...
 */
public class TrashDAO extends DAO<ObjetoLista> implements IListaDAO {

    private static final String TABLE = "Lista";

    public TrashDAO(Context ctx) {
        super(ctx);
    }
    ...
    public long deleteList(List<ObjetoLista> list) {
        ...
        for (ObjetoLista objetoLista : list) {
            if (objetoLista.getId() > 0) {
                String[] whereArgs = new String[] { String.valueOf(objetoLista.getId()) };
                getDb().delete("Item", "list_id=?", whereArgs);
    ...
    protected ContentValues toContentValue(ObjetoLista vo) {
        ...
        ctv.put("create_date", vo.getCreateDate());
        ctv.put("update_date", vo.getUpdateDate());
